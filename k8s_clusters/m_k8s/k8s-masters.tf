module "cloudinit" {
  ...
  unique_name		= "m_k8s_roce_ubuntu_2004_cloudinit.iso"
...
resource "libvirt_volume" "masters_data_disks" {
  pool 			= "nfsdatastore"
  format		= "raw"
  name			= "m_k8s_roce_master_0${count.index}.raw"
  size			= 25769803776
  count			= "${var.masters_vms_count}"
}

resource "libvirt_volume" "masters_ubuntu2004_boot_disk" {
  name			= "m-k8s-roce-master-0${count.index}.raw"
...
resource "libvirt_domain" "m_k8s_roce_masters" {
  name			= "m-k8s-roce-master-0${count.index}.kd.local"
  memory		= 2048
  vcpu			= 2
  count			= "${var.masters_vms_count}"
...
output "masters_ips" {
  value = "${libvirt_domain.m_k8s_roce_masters.*.network_interface.0.addresses}"
}
