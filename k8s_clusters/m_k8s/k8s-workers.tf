...
resource "libvirt_volume" "workers_data_Disks" {
  pool 			= "nfsdatastore"
  format		= "raw"
  name			= "m_k8s_roce_worker_0${count.index}.raw"
  size			= 25769803776
  count			= "${var.workers_vms_count}"
}

resource "libvirt_volume" "workers_ubuntu2004_boot_disk" {
  name			= "m-k8s-roce-worker-0${count.index}.raw"
...
resource "libvirt_domain" "m_k8s_roce_workers" {
  name			= "m-k8s-roce-worker-0${count.index}.kd.local"
  memory		= 8192
  vcpu			= 6
  count			= "${var.workers_vms_count}"
...
output "workers_ips" {
  value = "${libvirt_domain.m_k8s_roce_workers.*.network_interface.0.addresses}"
}
