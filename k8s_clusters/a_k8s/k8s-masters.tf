module "cloudinit" {
  ...
  unique_name		= "a_k8s_ubuntu_2004_cloudinit.iso"
...
resource "libvirt_domain" "a_k8s_roce_masters" {
  name			= "a-k8s-roce-master-0${count.index}.kd.local"
  memory		= 2048
  vcpu			= 2
  count			= "${var.masters_vms_count}"
...
output "masters_ips" {
  value = "${libvirt_domain.a_k8s_roce_masters.*.network_interface.0.addresses}"
}
