module "cloudinit" {
  ...
  unique_name		= "mesos_ubuntu_2004_cloudinit.iso"
...
resource "libvirt_domain" "mesos_masters" {
  name			= "mesos-master-0${count.index}.kd.local"
  memory		= 4096
  vcpu			= 2
  count			= "${var.masters_vms_count}"
...
output "masters_ips" {
  value = "${libvirt_domain.mesos_masters.*.network_interface.0.addresses}"
}
